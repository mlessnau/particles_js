...
Fov.prototype.initializeEventHandlers = function () {
  ...
  this.eventMediator.subscribe("fov:translate:decrement-x", function () { self.translate.x -= 10 / self.scale; self.setChanged(); });
  this.eventMediator.subscribe("fov:translate:increment-x", function () { self.translate.x += 10 / self.scale; self.setChanged(); });
  this.eventMediator.subscribe("fov:translate:reset", function () { self.translate = {x: 0, y: 0}; self.setChanged(); });
  this.eventMediator.subscribe("fov:translate:decrement-y", function () { self.translate.y -= 10 / self.scale; self.setChanged(); });
  this.eventMediator.subscribe("fov:translate:increment-y", function () { self.translate.y += 10 / self.scale; self.setChanged(); });
